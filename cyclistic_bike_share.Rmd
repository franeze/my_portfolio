...
**Note 4**: Merging monthly data in a single data frame
...
##Cleaning Data \#### **Initial Data Inspection** First, we need to
confirm the structure of the data.

```{r checking original data structure}
head(cyclistic_bike_share)
skim_without_charts(cyclistic_bike_share)
summary(cyclistic_bike_share)
# View(cyclistic_bike_share) #remove # to run the code
```

#### **Problems Identified**:

1 - The dates and times are in character format and need to be changed
for later calculations. 2 - The started_at and ended_at column names are
not clear enough, so they need to be changed. 3 - It is not easy to
determine how long each trip lasted, so we will define a new column with
this information, dropping negative values. 4 - Need to define the days
of the week and months. 5 - Station names and IDs are missing. If the
information of name or ID of a station is present, it's possible to
track its ID or name respectively. But if there is no information, the
data is useless because the coordinates don't match with the station in
several cases, so the data has to be filtered. 6 - Where the data is
incomplete, it will be dropped to avoid mistakes. We want no duplicates,
so if there are any duplicates, we will drop them with distinct(). To
make sure that the column names are unique and consistent, we will use
...
Now that we have our data set cleaned, we need to check if everything is
okay.

```{r checking clean data structure}
head(all_trips)
skim_without_charts(all_trips)
summary(all_trips)
#View(all_trips) #remove # to run the code
```
...
```{r}
all_trips %>% 
   group_by(client, day_of_week) %>%  #groups by user type and weekday
...
#### Now let's check the summaries

View the summary

```{r}
#View(week_summary) #remove # to run the code

#View(monthly_summary) #remove # to run the code

#View(monthly_week_summary) #remove # to run the code
...
Create a csv files to visualize in Tableau, or other presentation
software
